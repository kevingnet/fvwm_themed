...
parse_color(char *arg, Color *c, int default_alpha)
{
  ...
  if (arg[0] == '#')
    offset = 1;
  memcpy(br, &arg[offset], 2);
  memcpy(bg, &arg[offset+2], 2);
  memcpy(bb, &arg[offset+4], 2);
  br[2] = '\0';
  bg[2] = '\0';
  bb[2] = '\0';
  fprintf(stderr, "rgb %s %s %s\n", br, bg, bb);

  int r = (int)strtol(br, NULL, 16);
  int g = (int)strtol(bg, NULL, 16);
  int b = (int)strtol(bb, NULL, 16);
...
parse_color2(char *arg, Color *c)
{
  ...
  memcpy(bg, &arg[offset+2], 2);
  memcpy(bb, &arg[offset+4], 2);
  memcpy(ba, &arg[offset+6], 2);
  br[2] = '\0';
  bg[2] = '\0';
  bb[2] = '\0';
  ba[2] = '\0';
  fprintf(stderr, "rgb %s %s %s %s\n", br, bg, bb, ba);

  int r = (int)strtol(br, NULL, 16);
  int g = (int)strtol(bg, NULL, 16);
  int b = (int)strtol(bb, NULL, 16);
  int a = (int)strtol(ba, NULL, 16);
  ...
  for (/* global */ screen = 0; screen < ScreenCount(display); screen++) {
    ...
    for (i = 1; i < argc; i++) {
      ...
      if (strcmp(argv[i], "-alpha") == 0) {
        ...
        double da = (double) 255.0;

        for (j = 0; j < 256; j++) {
          r[j] = (DATA8) (((double) r[j] / da) * (double) c.r);
          g[j] = (DATA8) (((double) g[j] / da) * (double) c.g);
          b[j] = (DATA8) (((double) b[j] / da) * (double) c.b);
